...
export default function CurriculumsPage() {
        ...
        data.map(async (curriculum: any) => {
          try {
            ...
            const curriculumWithHierarchy =
              await curriculumAPI.getCurriculumWithHierarchy(curriculum.id);
  ...
  const handleCurriculumClick = (curriculum: any) => {
    setSelectedCurriculum(
      selectedCurriculum?.id === curriculum.id ? null : curriculum
    );
  };
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage your educational curricula
            </p>
          </div>
          <button
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
            ...
            <ul className="divide-y divide-gray-200">
              {isLoading ? (
                    ...
                    {selectedCurriculum?.id === curriculum.id && (
                      <div className="bg-gray-50 px-6 py-5 border-t border-gray-200 transition-all duration-300 animate-fadeIn">
                        ...
                        curriculum.subjects.length > 0 ? (
                          curriculum.subjects.map((subject: any) => (
                            <div
                              ...
                              <h3 className="font-medium text-gray-800 mb-3 flex items-center">
                                <span className="bg-gray-200 h-6 w-6 rounded-full flex items-center justify-center mr-2 text-sm text-gray-700 font-semibold">
                                  S
                                </span>
                              ...
                              {subject.courses && subject.courses.length > 0 ? (
                                subject.courses.map((course: any) => (
                                  <div
                                    key={course.id}
                                    className="ml-5 mb-4 border-l-2 border-gray-200 pl-4">
                                    <h4 className="font-medium text-gray-800 mb-3 flex items-center">
                                      <span className="bg-gray-100 h-6 w-6 rounded-full flex items-center justify-center mr-2 text-sm text-gray-700 font-semibold">
                                        C
                                      </span>
                          ...
                          <div className="bg-white p-4 rounded-lg text-center">
                            ...
                            <p className="text-sm text-gray-400 mt-1">
                              Use the curriculum editor to add subjects and
                              courses.
                            </p>
                          </div>
                        )}
                        ...
                        <div className="flex justify-end mt-4">
                          <Link
                            href={`/curricula/edit/${curriculum.id}`}
                            className="flex items-center gap-1.5 text-gray-600 hover:text-gray-800 font-medium mr-6 px-3 py-1.5 rounded hover:bg-gray-100 transition-colors duration-200">
                            <Edit size={16} />
                            <span>Edit</span>
                          </Link>
                          <button
                ...
                <li className="px-6 py-10 text-center text-gray-500">
                  No curricula found. Try changing your search query or add a
                  new curriculum.
                </li>
              )}
            </ul>
